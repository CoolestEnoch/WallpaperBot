...
fun checkUpdate(
	...
	thread {
		try {
			...
			if (!startup && (releaseObjectJson.has("message") && releaseObjectJson.getString("message")
					.equals("Not Found"))
			) {
				//没有最新release或没有上传
				Snackbar.make(
					...
					val cardView = ViewUtils_Kotlin.createBigButton(
						...
						for (file in latestRelease.relAssets) {
							...
							val fileCard = ViewUtils_Kotlin.createBigButton(
								context,null, R.color.item_card_bg,
								listOf(file[0], file[2], file[3]),null,R.color.white
							) {
			...
			if (!startup && (e.toString().contains("Unable to resolve host") || e.toString()
					.contains("failed to connect to"))
			) {
				Snackbar.make(
					swipeRefreshLayout.rootView,
					"检查更新失败: 无法连接到GitHub服务器, 请检查网络",
					Snackbar.LENGTH_LONG
				).show()
			} else {
				e.printStackTrace()
			...
			if (responseArray.length() == 0) {
				...
				for (release in releaseList) {
					...
					val cardView = ViewUtils_Kotlin.createBigButton(
						...
						runOnMainThread {
							ViewUtils_Kotlin.getBigCardFromBottom(context, "文件", relLinearLayout).show()
						}
