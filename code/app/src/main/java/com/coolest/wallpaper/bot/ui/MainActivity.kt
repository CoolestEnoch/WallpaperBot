...
//			WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
        ...
        when (MyApplication.firstStartUp) {
            ...
            false -> {
                ...
                getNewPic(this@MainActivity, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip, startup = true)
            }
            //转屏
            true -> try {
        ...
        binding.swipeRefresh.apply {
            ...
            setOnRefreshListener {
                when (lsp_enabled) {
                    // 正常模式
                    false -> getNewPic(this@MainActivity, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip)
                    ...
                    true -> {
                        ...
                        when {
                            lsp_count < lsp_count_max -> {
                                getNewPic(this@MainActivity, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.sharePic -> share(this, getCurrentImageBitmap(), getCurrentImageFileName(), getCurrentImageURL())
                ...
                null -> Snackbar.make(window.decorView, "图片未加载好, 稍后再试", Snackbar.LENGTH_LONG).show()
                    ...
                    val btnCancel = ViewUtils_Kotlin.createBigButton(
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            wallpaperManager.setBitmap(getCurrentImageBitmap(), null, true, WallpaperManager.FLAG_LOCK)
                        } else {
                            val mWallpaperManager = WallpaperManager.getInstance(this)
                            val mWallpaperManagerClass = mWallpaperManager.javaClass
                            val setLockScreenMethod = mWallpaperManagerClass.getMethod("setBitmapToLockWallpaper", Bitmap::class.java)
        ...
        when (requestCode) {
            QR_REQUEST_CODE -> {
                ...
                if (url.endsWith(".png") || url.endsWith(".jpg") || url.endsWith(".webp")) {
                    getNewPic(this@MainActivity, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip, false, url)
                }
            }
        }
    }
...
// to play the audio track.
    ...
    private fun getNewPic(context: Context, refreshLayout: SwipeRefreshLayout, imageView: ImageView, picLoadingTip: TextView, startup: Boolean = false, customURL: String? = null) {
        refreshBingWallpaper()
    ...
    fun selectChannel(context: Context, customTitle: String? = null, startup: Boolean = false) {
        ...
        for ((tag, channel) in channels) {
            val btnChannel = ViewUtils_Kotlin.createBigButton(
                ...
                getNewPic(context, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip)
                bottomBar?.cancel()
                if (startup) getNewPic(this@MainActivity, binding.swipeRefresh, binding.mainImageView, binding.picLoadingTip, startup = startup)
                Unit
            }
            list.add(btnChannel)
        }
                    ...
                    true -> {
                        ...
                        val btnQR = ViewUtils_Kotlin.createBigButton(
                            ...
                            try {
                                ...
                                Glide.with(context)
                                        .asBitmap()
                                        .load(BarcodeEncoder().encodeBitmap(getCurrentImageURL(), BarcodeFormat.QR_CODE, ViewUtils_Kotlin.dp2px(context, 350), ViewUtils_Kotlin.dp2px(context, 350)))
                                        .into(qrImg)
                            ...
                            ViewUtils_Kotlin.getBigCardFromBottom(context, qrTitle, qrBaseLayout).show()
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            val btnScanOneScan = ViewUtils_Kotlin.createBigButton(
                ...
                if (checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    ...
                    // 有权限，开启相机
                    startActivityForResult(Intent(this@MainActivity, CaptureActivity::class.java), QR_REQUEST_CODE)
                }
                bottomBar?.cancel()
                Unit
            }
