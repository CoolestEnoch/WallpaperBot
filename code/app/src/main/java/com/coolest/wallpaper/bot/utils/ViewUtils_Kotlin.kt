...
object ViewUtils_Kotlin {
    ...
    fun getBigCardFromBottom(context: Context?, title: String?, viewInCard: View?): MaterialDialog {
        ...
        //dialog.positiveButton(null, "确定") { materialDialog: MaterialDialog? -> null }
        dialog.negativeButton(null, "取消") { materialDialog: MaterialDialog -> null }
        return dialog
    }
    ...
    fun createCardView(
        ...
        cardMain.setCardBackgroundColor(context.resources.getColor(bg_color_id))
    ...
    fun createBigButton(
        ...
        when (strList) {
            null -> {}
                ...
                strLinearLayout.setPadding(dp2px(context, 16), dp2px(context, 25), 0, dp2px(context, 25))
                var firstEle = true //实现第一个文本框加粗
                strList.forEach { str ->
                    val tv = TextView(context)
                    val layoutParams = LinearLayout.LayoutParams(
                    ...
                    var color: Int
                    try {
                        color = context.resources.getColor(text_color_id_or_rgb_value)
                    } catch (e: java.lang.Exception) {
                        color = text_color_id_or_rgb_value
                    }
